...
isSorted xs =
  case length xs of
    0 -> property True
    1 -> property True
    _ -> property $ and [xs !! i <= xs !! (i + 1) | i <- [0 .. (length xs - 2)]]
...
sumIsLength input output = (length input) === (sumOuput output)
  where
    sumOuput ((c, freq):xs) = freq + sumOuput xs
    sumOuput []             = 0
...
inputInOutput input output =
  forAll (elements input) (\x -> any (\(y, _) -> x == y) output)
...
outputInInput input output =
  forAll
    (elements output)
    (\(x, count) -> (length $ filter (== x) input) === count)
...
frequenciesProp freq (NonEmpty input) =
  let output = freq input
   in sumIsLength input output .&&. inputInOutput input output .&&.
      outputInInput input output
...
genList = do
  n <- choose (3, 5)
  unsorted <- vectorOf n (choose (0, 10))
...
instance Arbitrary Expression where
  arbitrary =
    oneof [Plus <$> arbitrary <*> arbitrary, Minus <$> arbitrary <*> arbitrary]
