...
greetText name =
  let processedName =
   ...
   in T.concat [T.pack "Hello, ", processedName, T.pack "!"]
...
shout str =
  let lst = T.words str
   in T.unwords $ process 0 lst
  where
    process n [] = []
    process n (s:lst)
      | even n = (T.toUpper s) : process (n + 1) lst
      | otherwise = s : process (n + 1) lst
...
longestRepeat text =
  if T.null text
    then 0
    else maximum $ 0 : map T.length (T.group text)
