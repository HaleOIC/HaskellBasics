...
appendAll myRef strs = do
  currentVal <- readIORef myRef
  let newVal = currentVal ++ concat strs
  writeIORef myRef newVal
...
swapIORefs myRef1 myRef2 = do
  v1 <- readIORef myRef1
  v2 <- readIORef myRef2
  writeIORef myRef2 v1
  writeIORef myRef1 v2
...
hSelectLines h nums = do
  ...
  return $ map (\i -> contents !! (i - 1)) nums
