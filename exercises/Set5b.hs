...
treeMax Empty = 0
treeMax (Node val lb rb) = max val $ max (treeMax lb) (treeMax rb)
...
allValues condition (Node val lb rb) =
  condition val && allValues condition lb && allValues condition rb
...
mapTree f (Node val lb rb) = Node (f val) (mapTree f lb) (mapTree f rb)
...
cull val (Node v lb rb) =
  if val == v
    then Empty
    else Node v (cull val lb) (cull val rb)
...
isOrdered (Node v lb rb) =
  allValues (< v) lb && allValues (> v) rb && isOrdered lb && isOrdered rb
...
walk [] (Node v lb rb) = Just v
walk (step:steps) (Node v lb rb) =
  case step of
    StepL -> walk steps lb
    StepR -> walk steps rb
...
set [] val (Node v lb rb) = Node val lb rb
set (step:steps) val (Node v lb rb) =
  case step of
    StepL -> Node v (set steps val lb) rb
    StepR -> Node v lb (set steps val rb)
