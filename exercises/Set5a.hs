...
get ud =
  case ud of
    IncreaseMode age -> age
    DecreaseMode age -> age
...
tick ud =
  case ud of
    IncreaseMode age -> IncreaseMode (age + 1)
    DecreaseMode age -> DecreaseMode (age - 1)
...
toggle ud =
  case ud of
    IncreaseMode age -> DecreaseMode age
    DecreaseMode age -> IncreaseMode age
...
rgb (Mix color1 color2) =
  map (\(x, y) -> (x + y) / 2) $ zip (rgb color1) (rgb color2)
...
toNat val
  | val == 0 = Just Zero
  | val < 0 = Nothing
  | otherwise = Just (toNatHelper val)

toNatHelper :: Int -> Nat
toNatHelper 0 = Zero
toNatHelper val = PlusOne (toNatHelper $ val - 1)
