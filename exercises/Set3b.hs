...
sums :: Int -> [Int]
sums i = sums_helper 1 0 i

sums_helper :: Int -> Int -> Int -> [Int]
sums_helper cur sum end =
  if cur <= end
    then sum + cur : sums_helper (cur + 1) (sum + cur) end
    else []
...
indexDefault xs i def =
  case (xs, i) of
    (x:xs, 0) -> x
    ([], _) -> def
    (x:xs, _) -> indexDefault xs (i - 1) def
...
sumsOfHelper xs sum =
  case xs of
    [] -> []
    (x:xs) -> x + sum : sumsOfHelper xs (sum + x)
